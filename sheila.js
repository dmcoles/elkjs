...
ElkJs.Sheila = function () {
    ...
    self.makeSnapshotData = function () {
        ...
        var icr = (interrupt_enable.massterIRQ ? 1 : 0) + (interrupt_enable.reset ? 2 : 0) + (interrupt_enable.display_end ? 4 : 0) + (interrupt_enable.rtc ? 8 : 0) + (interrupt_enable.receive ? 0x10 : 0) + (interrupt_enable.transmit ? 0x20 : 0) + (interrupt_enable.high_tone ? 0x40 : 0);
        bytes.push(icr);

        bytes.push(self.read(0xfe00));

        bytes.push(screenaddrLo);
        bytes.push(screenaddrHi);
        bytes.push(tapeData);
        bytes.push(fe05);
        bytes.push(self.romBank);
        bytes.push(fe06);
        bytes.push(fe07);
        bytes.push(paletteRegs[0]);
        bytes.push(paletteRegs[1]);
        bytes.push(paletteRegs[2]);
        bytes.push(paletteRegs[3]);
        bytes.push(paletteRegs[4]);
        bytes.push(paletteRegs[5]);
        bytes.push(paletteRegs[6]);
        bytes.push(paletteRegs[7]);
