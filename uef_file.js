...
ElkJs.UEFFile = function(opts) {
	...
	function processUEF(arrayBuffer) {
		...
		if (tapeLoaded & autoLoad) {
			//somehow force a load
		}
	
	}
	
	self.loadUEF = function(file) {
		if (Object.prototype.toString.call(file)=='[object File]')
		{
			load_filereader(file,processUEF);
		}
		else {
			load_binary_resource(file,processUEF)
		}
	}
	...
	self.process = function() {
		...
		switch(tapeData[tapePos].type) {
			case "carrier":
				...
				currentTime++;
				if (currentTime>=tapeData[tapePos].cycleCount*13/multiplier) processNext = true;
				break;

			case "implicitDataBlock":
				highTone = false;
				currentTime++;
				if (currentTime>=13/multiplier) {
			...
			case "integerGap":
				currentTime++;
				if (currentTime>=tapeData[tapePos].gapTime*13/multiplier) {
					processNext = true;
				}
				break;
